...
public class GenericObjectVisitor implements IUnrealObjectVisitor {
	...
	private UnrealName nextPropertyOrKey;
	...
	public GenericObjectVisitor(GenericObject rootObject) {
		this.rootObject = rootObject;
	}

	public GenericObject getRootObject() {
		return rootObject;
	}
	...
	public void visitUnparseableData(ByteBuffer data) {
		...
		if (stateStack.peek() == VisitorState.PROPERTY_NAME) {
			((GenericObject) objectStack.peek()).properties.put(NATIVE_VARS, copy);
		} else {
			visitValue(copy);
		}
	}
	...
	private void visitValue(Object value) {
		...
		if (state == VisitorState.PROPERTY_VALUE) {
			...
			if (nextStaticArrayIndex > 0 && !(existingValue instanceof List)) {
				...
				existingValue = list;
				properties.put(nextPropertyOrKey, list);
			}
			...
			if (existingValue instanceof List && !(value instanceof List)) {
				...
				properties.put(nextPropertyOrKey, value);
			}
			...
			nextPropertyOrKey = UnrealName.from(value);
